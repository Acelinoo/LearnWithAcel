...
import { getVibeLesson } from "@/lib/vibeContent";
import { getVibeLevelBySlug } from "@/lib/vibeRoadmap";

export async function generateMetadata({ params }) {
  const lesson = getVibeLesson(params.level, params.lesson);
  if (!lesson) {
    const level = getVibeLevelBySlug(params.level);
    return {
      title: level ? `${level.title} — Coming soon` : "Materi Vibe Coding",
...
export default function VibeLessonPage({ params }) {
  const level = getVibeLevelBySlug(params.level);
  if (!level) notFound();

  const lesson = getVibeLesson(params.level, params.lesson);
  const lessonMeta = level.lessons.find((l) => l.slug === params.lesson);
  const viewerKey = `vibe/${params.level}/${params.lesson}`;
        ...
        <Reveal delay={0.05}>
          <div className="mt-12 flex flex-col items-center text-center">
            ...
            <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
              <Link href={`/roadmap/vibe#${level.slug}`} className="btn-primary">
                Lihat materi lain
                <ArrowRight size={16} />
              </Link>
